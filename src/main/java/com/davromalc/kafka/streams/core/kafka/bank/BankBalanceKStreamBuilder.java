...
import com.davromalc.kafka.streams.bootstrap.bank.TransactionJsonSerde;
...
public class BankBalanceKStreamBuilder {
	...
	public KStream<String, Transaction> build() throws ReflectionException {
		...
		stream
				...
				.toStream().to(outputTopic, Produced.with(keySerde, valueSerde));
	...
	Balance applyTransaction(final Balance balance, final Transaction transaction) {
		final BigDecimal amount = balance.getAmount().add(BigDecimal.valueOf(transaction.getAmount()));
